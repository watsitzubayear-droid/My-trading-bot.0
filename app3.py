...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("**🔗 [Open Quotex](https://quotex.com/en/trade)**")
...
def main_bot_interface(selected_pairs):
    ...
    if st.session_state.dark_mode:
        ...
        st.markdown("""
            <style>
            .signal-box { background-color: #f0f2f6; padding: 20px; border-radius: 10px; }
            </style>
        """, unsafe_allow_html=True)
    ...
    while st.session_state.running:
        ...
        with status_placeholder:
            st.info(f"🔄 Cycle #{cycle_count} | Analyzing selected pairs...")
        
        # Analyze all pairs
        try:
            all_results = analyze_all_pairs(selected_pairs)
            
            if not all_results:
                with status_placeholder:
                    st.error("No valid results from analysis. Check error log.")
                time.sleep(60)
                continue
                    ...
                    <div class="signal-box">
                        ...
                        <a href="https://quotex.com/en/trade/{best_signal['pair'].replace('/', '')}" target="_blank">
                            <button style="background-color: #4CAF50; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer;">
                                📊 Open {best_signal['pair']} on Quotex →
                            </button>
                        </a>
                    </div>
                """, unsafe_allow_html=True)
            ...
            st.session_state.errors.append(f"Cycle {cycle_count} error: {e}")
            with status_placeholder:
                st.error(f"Cycle {cycle_count} failed. Retrying...")
            time.sleep(30)
            continue
